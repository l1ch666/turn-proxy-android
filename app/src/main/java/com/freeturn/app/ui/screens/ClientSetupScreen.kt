...
fun ClientSetupScreen(
    ...
    val fullTunnelUriValidation = remember(fullTunnelClientUri, tunnelMode) {
        if (tunnelMode == TunnelMode.FULL_TUNNEL) {
            FullTunnelUriValidator.validate(fullTunnelClientUri)
        } else {
            FullTunnelUriValidator.validate("vless://placeholder@127.0.0.1:1")
        }
    }
    ...
    LaunchedEffect(serverAddress, vkLink, threads, streamsPerCred, useUdp, manualCaptcha, useCarrierDns, localPort, dnsMode, forcePort443, debugMode, magicSwitch, magicTurn, tunnelMode, fullTunnelClientUri, enableVlessBond) {
        ...
        viewModel.saveClientConfig(
            ClientConfig(
                ...
                fullTunnelClientUri = fullTunnelClientUri.trim()
            )
        )
    }
        ...
        Column(
            ...
            Column(
                ...
                Column(verticalArrangement = Arrangement.spacedBy(8.dp)) {
                    Column {
                        Text(stringResource(R.string.tunnel_mode_title), style = MaterialTheme.typography.bodyMedium)
                    }
                    SingleChoiceSegmentedButtonRow(modifier = Modifier.fillMaxWidth()) {
                        TunnelMode.entries.forEachIndexed { idx, value ->
                            SegmentedButton(
                                selected = tunnelMode == value,
                                onClick = {
                ...
                if (tunnelMode == TunnelMode.FULL_TUNNEL) {
                    val uriMessage = when {
                        !fullTunnelUriValidation.isValid ->
                            fullTunnelUriValidation.errors.joinToString("; ")
                        fullTunnelUriValidation.warnings.isNotEmpty() ->
                            fullTunnelUriValidation.warnings.joinToString("; ")
                        else -> stringResource(R.string.full_tunnel_client_uri_support)
                    }
                    OutlinedTextField(
                        value = fullTunnelClientUri.redact(privacyMode),
                        onValueChange = { if (!privacyMode) fullTunnelClientUri = it },
                        label = { Text(stringResource(R.string.full_tunnel_client_uri_label)) },
                        placeholder = { Text(stringResource(R.string.full_tunnel_client_uri_placeholder)) },
                        modifier = Modifier.fillMaxWidth(),
                        singleLine = false,
                        minLines = 2,
                        maxLines = 4,
                        readOnly = privacyMode,
                        isError = !fullTunnelUriValidation.isValid,
                        supportingText = { Text(uriMessage) }
                    )
                }
