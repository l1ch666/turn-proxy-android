...
data class ClientConfig(
    ...
    val tunnelMode: TunnelMode = TunnelMode.LOCAL_PROXY,
    val fullTunnelClientUri: String = ""
)
...
class AppPreferences(context: Context) {
    ...
    companion object {
        ...
        val CLIENT_TUNNEL_MODE = stringPreferencesKey("client_tunnel_mode")
        val CLIENT_FULL_TUNNEL_REV = intPreferencesKey("client_full_tunnel_rev")
        private const val CLIENT_FULL_TUNNEL_URI_SECURE = "client_full_tunnel_uri"
    ...
    val clientConfigFlow: Flow<ClientConfig> = context.dataStore.data
        ...
        .map { prefs ->
            ClientConfig(
                ...
                tunnelMode = TunnelMode.fromStorage(prefs[CLIENT_TUNNEL_MODE]),
                fullTunnelClientUri = encryptedPrefs.getString(CLIENT_FULL_TUNNEL_URI_SECURE, null) ?: ""
            )
        }
    ...
    suspend fun saveClientConfig(config: ClientConfig) {
        withContext(Dispatchers.IO) {
            encryptedPrefs.edit {
                putString(CLIENT_FULL_TUNNEL_URI_SECURE, config.fullTunnelClientUri.trim())
            }
        }
        context.dataStore.edit { prefs ->
            ...
            prefs[CLIENT_TUNNEL_MODE] = config.tunnelMode.name
            prefs[CLIENT_FULL_TUNNEL_REV] = (prefs[CLIENT_FULL_TUNNEL_REV] ?: 0) + 1
